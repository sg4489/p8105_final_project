...
### line plot for year (Decide to delete this)
```{r year trend in regions, warning=FALSE, message=FALSE}
region_year_trend_lineplot = 
  region_population_df |> 
  select(residence_census_region, year_code, deaths, population) |>
  group_by(residence_census_region, year_code) |>
  summarise(
  ...
  ungroup() |>
  slice(-n()) |> 
  ggplot(aes(x = year_code, y = death_rate, color = residence_census_region)) +
  geom_point() + 
  geom_line(size = 0.8, alpha = 0.7) +
  labs(title = "Death Rate by Region (2018-2024)",
       x = "Year",
