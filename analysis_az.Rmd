...
## how did covid death rate vary across states over time?
```{r}
...
data_covid_df = data_covid_states %>%
  ...
  mutate(
    ...
    covid_mortality_per10000 = covid_mortality*10000) 
```
...
fig = plot_ly(
  ...
  z = ~covid_mortality_per10000, # Color scale based on covid_mortality
  text = ~paste("State: ", state), # Hover text
  colorscale = "Oranges",
  colorbar = list(title = "Mortality Rate per 10000")
) %>%
  layout(
